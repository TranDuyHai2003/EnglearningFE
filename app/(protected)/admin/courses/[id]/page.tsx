...
export default function AdminCourseDetailPage() {
  ...
  const fetchCourse = async () => {
    try {
      ...
      // Update selected lesson if it exists to reflect changes
      if (selectedLesson) {
    ...
    <div className="h-[calc(100vh-4rem)] flex flex-col">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-80 border-r bg-slate-50 flex flex-col shrink-0">
            <div className="p-4 border-b bg-white">
                ...
                <div className="text-xs text-muted-foreground flex gap-2">
                    <span className="flex items-center gap-1"><CheckCircle className="h-3 w-3 text-green-600"/> Đã duyệt</span>
                    <span className="flex items-center gap-1"><XCircle className="h-3 w-3 text-red-600"/> Từ chối</span>
                    <span className="flex items-center gap-1"><Clock className="h-3 w-3 text-slate-500"/> Chờ duyệt</span>
                </div>
            </div>
                ...
                <div className="p-4 space-y-4">
                    <button
                        ...
                        className={cn(
                            ...
                            !selectedLesson ? "bg-primary/10 text-primary" : "hover:bg-slate-200"
                        )}
                    >
                                ...
                                {section.lessons?.map((lesson) => (
                                    ...
                                    >
                                        <div className="flex items-center gap-2 overflow-hidden">
                                            {lesson.lesson_type === "video" && <PlayCircle className="h-3 w-3 shrink-0" />}
                                            {lesson.lesson_type === "document" && <FileText className="h-3 w-3 shrink-0" />}
                                            {lesson.lesson_type === "quiz" && <HelpCircle className="h-3 w-3 shrink-0" />}
                                            <span>{lesson.title}</span>
                                        </div>
                                        <div className="shrink-0">
                                            {lesson.approval_status === "approved" && <CheckCircle className="h-3 w-3 text-green-600" />}
                                            {lesson.approval_status === "rejected" && <XCircle className="h-3 w-3 text-red-600" />}
                                            {lesson.approval_status === "pending" && <div className="h-2 w-2 rounded-full bg-orange-400" />}
                                        </div>
                                    </button>
                                ))}
                            </div>
        ...
        <div className="flex-1 overflow-y-auto bg-white p-6">
            {selectedLesson ? (
                <AdminLessonPreview lesson={selectedLesson} onUpdate={fetchCourse} />
            ) : (
                <div className="max-w-4xl mx-auto space-y-8">
                    ...
                    <div className="aspect-video relative rounded-lg overflow-hidden bg-slate-100 shadow-sm">
                        {course.thumbnail_url ? (
                            ...
                            <div className="flex items-center justify-center h-full text-muted-foreground">Không có ảnh bìa</div>
                        )}
                    </div>
                            ...
                            <Card>
                                ...
                                <CardContent className="space-y-4 text-sm">
                                    ...
                                    <div className="flex justify-between">
                                        ...
                                        <span className="font-medium">{course.discount_price?.toLocaleString() || 0} đ</span>
                                    </div>
                                    <div className="flex justify-between">
